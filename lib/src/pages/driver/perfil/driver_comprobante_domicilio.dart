...
class ComprobanteDomicilioPage extends StatefulWidget {
  final String userId;
  
  const ComprobanteDomicilioPage({
    super.key,
    required this.userId
  });

  @override
  State<ComprobanteDomicilioPage> createState() => _ComprobanteDomicilioPageState();
}

class _ComprobanteDomicilioPageState extends State<ComprobanteDomicilioPage> {
  
  final _paletaColors = PaletaColors();
  final ImagePicker picker = ImagePicker();
  GlobalKey<ScaffoldState> key = GlobalKey<ScaffoldState>();

  String urlFotoDomicilio = '';
  String userId = '';
  bool cargando = true;
  ...
  void _initData() async {
    ...
    if(datos.data() != null){
      ...
      urlFotoDomicilio = datosUser.containsKey('domicilio') ? datosUser['domicilio'] : '';
    }
      ...
      child: Column(
        children: [
          Align(
            ...
            child: _tituloSeccion('Imágen comprobante domicilio', 17.5)
          ),
          _espacio(),
          _imageMarco(),
          _botonGuardar()
  ...
  Widget _image(){
    ...
    if(urlFotoDomicilio.isNotEmpty){
      return _imageClip(
        Image.network(
          urlFotoDomicilio,
          fit: BoxFit.cover,
  ...
  void _crearRegistro() async {
    ...
    MisAvisos().ventanaTrabajando(context, 'Actualizando comprobante...');
    ...
    try {
      ...
      MisAvisos().showToast('¡Comprobante actualizado!', 2000, fToast!, false);
      if(mounted) Navigator.pop(context, {'newComprobanteDomicilio': 1});
