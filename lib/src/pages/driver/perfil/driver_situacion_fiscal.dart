...
class ComprobanteFiscalPage extends StatefulWidget {
  final String userId;
  
  const ComprobanteFiscalPage({
    super.key,
    required this.userId
  });

  @override
  State<ComprobanteFiscalPage> createState() => _ComprobanteFiscalPageState();
}

class _ComprobanteFiscalPageState extends State<ComprobanteFiscalPage> {
  
  final _paletaColors = PaletaColors();
  final ImagePicker picker = ImagePicker();
  GlobalKey<ScaffoldState> key = GlobalKey<ScaffoldState>();

  String urlFotoConstancia = '';
  String userId = '';
  bool cargando = true;
      ...
      child: Column(
        children: [
          Align(
            ...
            child: _tituloSeccion('Imágen constancia', 17.5)
          ),
          _espacio(),
          _imageMarco(),
          _botonGuardar()
  ...
  Widget _image(){
    ...
    if(urlFotoConstancia.isNotEmpty){
      return _imageClip(
        Image.network(
          urlFotoConstancia,
          fit: BoxFit.cover,
  ...
  void _crearRegistro() async {
    ...
    MisAvisos().ventanaTrabajando(context, 'Actualizando constancia...');
    ...
    try {
      ...
      MisAvisos().showToast('¡Constancia actualizada!', 2000, fToast!, false);
      if(mounted) Navigator.pop(context, {'newfiscal': 1});
